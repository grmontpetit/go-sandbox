...
func main() {
	handler := func(w http.ResponseWriter, r *http.Request) {
		cycles, err := extractQueryInt(r.URL, "cycles")
		if err != nil {
			log.Printf("Error parsing URL: %q", err)
			cycles = 1
		}
		lissajous(w, cycles)
	}
...
func extractQueryInt(path *url.URL, paramName string) (int, error) {
	values := path.Query()
	if len(values["cycles"]) == 0 {
		return 0, errors.New("Cycles where no specified")
	}
	return strconv.Atoi(values["cycles"][0])
}
